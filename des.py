...
def left_shift(bits, n):
    """Left shift bit string by n positions"""
...
def des_decrypt(ciphertext_bin, round_keys):
    """DES Decryption Algorithm"""
    # Initial Permutation
    ciphertext = permute(ciphertext_bin, IP)
    l_prev = ciphertext[:32]
    r_prev = ciphertext[32:]
    
    # Reverse round keys for decryption
    reversed_keys = reversed(round_keys)
    
    for i, key in enumerate(reversed_keys):
        # Expansion
        expanded_r = permute(r_prev, E)
        # XOR with round key
        xored = ''.join(str(int(a) ^ int(b)) for a, b in zip(expanded_r, key))
        # S-Box substitution
        s_out = []
        for j in range(8):
            block = xored[j*6:(j+1)*6]
            row = int(block[0] + block[5], 2)
            col = int(block[1:5], 2)
            val = S_BOX[j][row][col]
            s_out.append(f"{val:04b}")
        s_out = ''.join(s_out)
        # Permutation
        p_result = permute(s_out, P)
        # XOR with left half
        new_r = ''.join(str(int(a) ^ int(b)) for a, b in zip(l_prev, p_result))
        # Update values
        l_prev, r_prev = r_prev, new_r
        
        # Print intermediate values
        print(f"Round {i+1}:")
        print(f"f Function Output: {p_result}")
        print(f"L: {l_prev}, R: {r_prev}\n")
    
    # Final swap and permutation
    combined = r_prev + l_prev
...
# Decrypt
print("\nDecryption Process:")
plaintext_bin = des_decrypt(ciphertext_bin, round_keys)
plaintext = binary_to_ascii(plaintext_bin)
